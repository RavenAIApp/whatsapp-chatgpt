...
async function handleIncomingMessage(message: Message) {
	...
	//if (true) {
		...
		// DALLE (!dalle <prompt>)
		else if (startsWithIgnoreCase(messageString, config.dallePrefix)) {
			const prompt = messageString.substring(config.dallePrefix.length + 1);
			await handleMessageDALLE(message, prompt);
			return;
		}
		// LLM (!llm <prompt>)
		else if (startsWithIgnoreCase(messageString, config.llmPrefix)) {
			const prompt = messageString.substring(config.llmPrefix.length + 1);
			await handleMessageLLM(message, prompt);
			return;
		}
		// SUMMARY (!summary <prompt>)
			...
			// LLM form docs
			await handleMessageLLMDocs(message);
			return;
		}
	}
}
